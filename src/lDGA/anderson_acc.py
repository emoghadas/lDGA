...
class AndersonAcceleration:
    ...
    taken from https://github.com/joshnguyen99/anderson_acceleration
    ...
    linmix_param : float (optional, default=0.5)
        Linear mixing parameter to be used for first few iterates before window_size is reached.
    ...
    def __init__(self, window_size=5, reg=0, mixing_param=1., linmix_param=0.5):
        ...
        assert 0 <= linmix_param <= 1, "Linear mixing parameter must be between 0 and 1"
        self.linmix_param = linmix_param
    ...
    def apply(self, x):
        ...
        if len(self.x_hist_) == 0:
            ...
            return x
        elif len(self.x_hist_) <= self.window_size:
            x_prev = self.x_hist_[-1]    
            x_linmix = (1-self.linmix_param) * x + self.linmix_param * x_prev
            self.x_hist_.append(x_linmix)
            return x_linmix
