...
def root_function_uniform(lambda_r:np.float64, beta:np.float64, chi_r_latt:np.ndarray, chi_r_loc:np.ndarray, lambda_maxpole:np.float64, weights:np.ndarray) -> np.float64:
    ...
    return np.sum( chi_r_loc - ( chi_r_latt/(1.0+lambda_r*chi_r_latt))@weights/np.sum(weights)  ).real/beta
...
def get_lambda_wdep(chi_r_latt:np.ndarray, chi_r_loc:np.ndarray, beta:np.float64, weights:np.ndarray) -> np.ndarray:
